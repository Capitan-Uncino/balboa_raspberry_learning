...
fn train_iql_actor(
    ...
    // --- Using SGD to prevent momentum reset shock upon resuming ---
    ...
    let epochs = 60; // Increased to 60 because SGD learns slightly slower than Adam
    let batch_size = 256;
    ...
    for epoch in 0..epochs {
        ...
        while start_idx < num_transitions {
            ...
            let actor_loss = (weight * -log_prob).mean();
    ...
    println!(">>> IQL Compilation Complete. Calculating data-driven anomaly thresholds...");

    // --- NEW: Calculate Data-Driven Confidence Thresholds from the Training Batch ---
    // 1. Pass the entire state dataset through the final Actor
    let pi_output = actor.forward(t_states.clone());
    let mu = pi_output.slice([0..num_transitions, 0..DIM_U]);
    let sa_tensor = Tensor::cat(vec![t_states, mu], 1);

    // 2. See how much the Critics disagree on these chosen actions
    let q1_vals = critic_q1.forward(sa_tensor.clone());
    let q2_vals = critic_q2.forward(sa_tensor);

    // 3. Compute Mean and Standard Deviation of the Disagreement
    let diff = (q1_vals - q2_vals).abs().detach();

    // Fix: Extract the mean as a standard Rust scalar first to avoid tensor dimension mismatches
    let mean_val = diff.clone().mean().into_scalar() as f64;

    // Subtract the scalar from the 2D tensor (Burn handles this cleanly)
    let diff_centered = diff - (mean_val as f32);

    // Square the centered differences, take the mean, and square root it for the std dev
    let std_val = diff_centered.powf_scalar(2.0).mean().sqrt().into_scalar() as f64;

    // Safe threshold: Mean - 1 STD (bounded to 0)
    let safe_diff = (mean_val - 0.7 * std_val).max(0.0);
    // Unsafe threshold: Mean + 1 STD
    let unsafe_diff = (mean_val - 0.2 * std_val).max(0.0);

    println!(
        ">>> Data-Driven Thresholds computed: Mean={:.4}, Std={:.4} | Safe={:.4}, Unsafe={:.4}",
        mean_val, std_val, safe_diff, unsafe_diff
    );
    ...
    // Return the dynamic thresholds along with the networks!
    (
        actor,
        critic_q1,
        critic_q2,
...
pub fn get_policy(batch: &[StateAction], current_policy: &Policy) -> Policy {
    ...
    // --- Extract Data-Driven Thresholds generated by the training loop ---
    let (actor, critic_q1, critic_q2, safe_diff, unsafe_diff, new_state) = train_iql_actor(
        t_states,
        t_actions,
        t_rewards,
        t_next_states,
    ...
    Policy::new(
        move |state_vec: &SVector<f64, DIM_X>| -> f64 {
            ...
            // 1. Get NN Prediction
            let action_tensor = infer_actor.forward(state_tensor.clone());
            let action_mean = action_tensor.slice([0..1, 0..DIM_U]);
            let nn_action = action_mean.clone().into_scalar() as f64;

            // 2. CRITIC DISAGREEMENT EVALUATION
            let sa_tensor = Tensor::cat(vec![state_tensor, action_mean], 1);

            let q1 = infer_q1.forward(sa_tensor.clone()).into_scalar() as f64;
            let q2 = infer_q2.forward(sa_tensor).into_scalar() as f64;

            let disagreement = (q1 - q2).abs();
