...
fn train_iql_actor(
...
) -> (MlpNetwork<B>, MlpNetwork<B>, MlpNetwork<B>, IqlModelState) {
    // <-- UPDATED SIGNATURE
    ...
    // Return the critics alongside the actor!
    (actor, critic_q1, critic_q2, new_state)
}
...
pub fn get_policy(batch: &[StateAction], current_policy: &Policy) -> Policy {
    ...
    // Extract both critics from the updated training function
    let (actor, critic_q1, critic_q2, new_state) = train_iql_actor(
        t_states,
        t_actions,
        t_rewards,
        t_next_states,
    ...
    Policy::new(
        move |state_vec: &SVector<f64, DIM_X>| -> f64 {
            ...
            // Slice out only the mean action
            ...
            // Concatenate State + Action along dimension 1
            ...
            // The mathematical anomaly score
            ...
            let safe_diff = 0.0008; // Below this difference, 100% NN
            let unsafe_diff = 0.005; // Above this difference, 100% Linear

            let mut raw_alpha = (unsafe_diff - disagreement) / (unsafe_diff - safe_diff);
            ...
            *p_alpha_guard = smoothed_alpha;
            ...
            //println!("diff: {:.4} | alpha: {:.4}", disagreement, smoothed_alpha);
