...
pub const POLYAK: f32 = 0.005; // Target network update rate
...
fn train_iql_actor(
    ...
    let mut target_value_v = value_v.clone();
    ...
    println!(">>> Starting IQL Optimization Loop...");
    let epochs = 60;
    let batch_size = 256;
    ...
    for _epoch in 0..epochs {
        ...
        while start_idx < num_transitions {
            ...
            let target_q = Tensor::min_pair(q1_val, q2_val);
            ...
            value_v = optim_v.step(
                LEARNING_RATE,
                value_v,
                GradientsParams::from_grads(grads_v, &value_v),
            );

            // --- 2. CRITIC UPDATE ---
            let v_next = target_value_v.forward(b_s_next);
            let q_target = b_r + v_next.mul_scalar(GAMMA);

            let current_q1 = critic_q1.forward(b_sa.clone());
            let q1_loss = (current_q1 - q_target.clone()).powf_scalar(2.0).mean();
            critic_q1 = optim_q1.step(
                LEARNING_RATE,
                critic_q1,
                GradientsParams::from_grads(q1_loss.backward(), &critic_q1),
            );

            let current_q2 = critic_q2.forward(b_sa);
            let q2_loss = (current_q2 - q_target).powf_scalar(2.0).mean();
            critic_q2 = optim_q2.step(
                LEARNING_RATE,
                critic_q2,
                GradientsParams::from_grads(q2_loss.backward(), &critic_q2),
            );

            // --- 3. ACTOR UPDATE ---
            let advantage = target_q - v_val;
            ...
            actor = optim_actor.step(
                LEARNING_RATE,
                actor,
                GradientsParams::from_grads(actor_loss.backward(), &actor),
            );

            // --- 4. POLYAK TARGET TRACKING ---
            target_value_v = target_value_v.fork(device);
